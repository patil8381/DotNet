...
Func<int,float,double,double> f=new Func<int,float,double,double>(p.Sam);
double d=f(12,14.14f,55.55);
Console.WriteLine(d);

Action<int,int,double> a=new Action<int,int,double>(p.Sammy);
a(12,14,55.55);

Predicate<string> pr=new Predicate<string>(p.Tom);
